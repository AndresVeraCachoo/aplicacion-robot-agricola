...
\begin{itemize}
    ...
    \item \textbf{Testcontainers \cite{testcontainers}:} Herramienta que soporta pruebas de integración mediante el despliegue programático de contenedores Docker \cite{docker} efímeros (como bases de datos desechables) durante la ejecución de los tests.
...
\begin{itemize}
    \item \textbf{Docker \cite{docker} y Docker Compose \cite{docker}:} Plataforma de contenerización que permite empaquetar aplicaciones junto con sus dependencias en unidades estándar aisladas. Docker Compose facilita la orquestación de múltiples contenedores interconectados mediante un único archivo declarativo.
...
\begin{itemize}
    ...
    \item \textbf{Docker y Containers (Microsoft):} Extensiones para la inspección visual, gestión y monitorización en tiempo real de los contenedores locales (\textit{Frontend}, \textit{Backend} y Base de Datos), agilizando las pruebas de despliegue y la orquestación del entorno.
